...
module.exports = function(options, callback) {
  ...
  function handleJavascriptFiles(jsCallback, noWatch, test) {
    test = test || options.test;
    var pluginContext = 'javascript',
        src = 'js/**/*' + (test ? '-test.js' : ''),
        callbackExecuted;

    if (options.watch && !noWatch && !test) {
      var watchGlobs = util.pluginGroupGlobs(pluginContext, options);
      watchGlobs.push(options.srcPath + src);
      util.src(watchGlobs, options)
          .pipe(newPlumber())
          .pipe(watch({}, function() {
            handleJavascriptFiles(!callbackExecuted && callback, true, false);
            handleJavascriptFiles(!callbackExecuted && callback, true, true);
            callbackExecuted = true;
          }));
      return;
    }

    var pipeline = gulp.src(options.srcPath + src);
    pipeline = pipeline.pipe(sectionEntryPoint(options, test));
    ...
    if (test) {
      pipeline = pipeline.pipe(gulp.dest('./build/test'));
    } else {
  ...
  function handleCSSFiles(cssCallback, noWatch) {
