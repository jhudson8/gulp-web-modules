...
function generateUserConfigInputHTML(userConfig) {
  ...
  html += (userConfig.inputs || []).map(function(entry) {
      ...
      subhtml += ('<label for="' + inputId + '">' + entry.label + '</label><br>');
      if (type === 'boolean') {
...
function showConfig(requestOptions, pluginOptions, callback) {
  ...
  var html = '<html><body><form action="/$admin/update">';
  html += userConfigEntries.map(generateUserConfigInputHTML).join('<br><br>');
